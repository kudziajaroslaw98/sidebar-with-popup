...
describe('StrengthBar', () => {
  ...
  test('renders the correct number of filled bars', () => {
    ...
    strengthPills.forEach((pill, index) => {
      expect(pill.className).toContain(index < strength ? 'bg-orange-500' : 'bg-gray-300');
    });
    ...
    strengthPills.forEach((pill, index) => {
      expect(pill.className).toContain(index < strength ? 'bg-teal-500' : 'bg-gray-300');
    });
    ...
    strengthPills.forEach((pill, index) => {
      expect(pill.className).toContain(index < strength ? 'bg-red-500' : 'bg-gray-300');
    });
  });
});
